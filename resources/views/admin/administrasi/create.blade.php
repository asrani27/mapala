...
@section('title', 'Tambah Administrasi — MAPALA Admin')

@section('content')
<div class="mb-8">
    <h1 class="font-display-lg text-headline-md text-primary uppercase">Tambah Surat</h1>
    <p class="font-body-md text-on-surface-variant mt-1">Tambah data administrasi baru.</p>
</div>

<div class="bg-surface rounded-lg border-2 border-outline p-6 max-w-2xl">
    <form method="POST" action="{{ route('admin.administrasi.store') }}" enctype="multipart/form-data">
        @csrf
        ...
        <div class="mb-8">
            ...
            <p class="mt-2 font-body-sm text-error">{{ $message }}</p>
            @enderror
            <p class="mt-2 font-body-sm text-on-surface-variant">Format: PDF, DOC, DOCX, JPG, PNG (Maks. 10MB)</p>
        </div>
