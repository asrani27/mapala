...
@section('title', 'Administrasi — MAPALA Admin')

@section('content')
<div class="mb-8 flex flex-col md:flex-row md:items-center md:justify-between gap-4">
    <div>
        <h1 class="font-display-lg text-headline-md text-primary uppercase">Administrasi</h1>
        <p class="font-body-md text-on-surface-variant mt-1">Kelola data administrasi surat.</p>
    </div>
...
<form method="GET" class="mb-6">
    <div class="flex gap-3">
        <input type="text" name="search" value="{{ $search }}" placeholder="Cari nomor surat, jenis, atau perihal..."
        ...
        <table class="w-full">
            <thead>
                <tr class="bg-surface-container-highest border-b-2 border-outline">
                    <th class="text-left px-6 py-4 font-label-caps text-label-caps text-primary">No. Surat</th>
                    <th class="text-left px-6 py-4 font-label-caps text-label-caps text-primary">Jenis Surat</th>
                    <th class="text-left px-6 py-4 font-label-caps text-label-caps text-primary">Tanggal</th>
                    <th class="text-left px-6 py-4 font-label-caps text-label-caps text-primary">Perihal</th>
            ...
            <tbody>
                ...
                <tr class="border-b border-outline-variant hover:bg-surface-container-low transition-colors">
                    <td class="px-6 py-4 font-body-md text-on-surface">{{ $a->nomor_surat }}</td>
                    <td class="px-6 py-4 font-body-sm text-on-surface-variant">{{ $a->jenis_surat }}</td>
                    <td class="px-6 py-4 font-body-sm text-on-surface-variant">{{ $a->tanggal_surat->format('d/m/Y') }}</td>
                    <td class="px-6 py-4 font-body-md text-on-surface">{{ $a->perihal }}</td>
                    <td class="px-6 py-4">
                        <div class="flex items-center justify-center gap-1">
                            <a href="{{ route('admin.administrasi.show', $a) }}"
                            ...
                            <a href="{{ route('admin.administrasi.edit', $a) }}"
                            ...
                            <button type="button" onclick="confirmDelete({{ $a->id }}, '{{ $a->nomor_surat }}')"
                ...
                <tr>
                    <td colspan="5" class="px-6 py-16 text-center">
                        <span class="material-symbols-outlined text-4xl text-outline mb-3 block">description</span>
                        <p class="font-body-md text-on-surface-variant">Tidak ada data administrasi ditemukan.</p>
                    </td>
                </tr>
                @endforelse
            </tbody>
