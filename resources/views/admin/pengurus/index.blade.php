...
@section('title', 'Pengurus — MAPALA Admin')

@section('content')
<div class="mb-8 flex flex-col md:flex-row md:items-center md:justify-between gap-4">
    <div>
        <h1 class="font-display-lg text-headline-md text-primary uppercase">Pengurus</h1>
        <p class="font-body-md text-on-surface-variant mt-1">Kelola data pengurus MAPALA.</p>
    </div>
...
<form method="GET" class="mb-6">
    <div class="flex gap-3">
        <input type="text" name="search" value="{{ $search }}" placeholder="Cari nama, NIM, jabatan, atau periode..."
        ...
        <table class="w-full">
            <thead>
                <tr class="bg-surface-container-highest border-b-2 border-outline">
                    ...
                    <th class="text-left px-6 py-4 font-label-caps text-label-caps text-primary">Jabatan</th>
                    <th class="text-left px-6 py-4 font-label-caps text-label-caps text-primary">Periode</th>
            ...
            <tbody>
                ...
                <tr class="border-b border-outline-variant hover:bg-surface-container-low transition-colors">
                    <td class="px-6 py-4 font-body-md text-on-surface">{{ $p->anggota->nama }}</td>
                    <td class="px-6 py-4 font-body-sm text-on-surface-variant">{{ $p->anggota->nim }}</td>
                    <td class="px-6 py-4 font-body-md text-on-surface">{{ $p->jabatan }}</td>
                    <td class="px-6 py-4 font-body-sm text-on-surface-variant">{{ $p->periode }}</td>
                    <td class="px-6 py-4">
                        <span class="inline-block font-label-caps text-label-caps px-3 py-1 rounded-full
                            {{ $p->status === 'aktif' ? 'bg-primary-container text-on-primary-container' : 'bg-error-container text-on-error-container' }}">
                            {{ ucfirst($p->status) }}
                        </span>
                    </td>
                    <td class="px-6 py-4">
                        <div class="flex items-center justify-center gap-1">
                            <a href="{{ route('admin.pengurus.show', $p) }}"
                            ...
                            <button type="button" onclick="confirmDelete({{ $p->id }}, '{{ $p->anggota->nama }} - {{ $p->jabatan }}')"
                ...
                <tr>
                    <td colspan="6" class="px-6 py-16 text-center">
                        <span class="material-symbols-outlined text-4xl text-outline mb-3 block">supervisor_account</span>
                        <p class="font-body-md text-on-surface-variant">Tidak ada pengurus ditemukan.</p>
                    </td>
                </tr>
                @endforelse
            </tbody>
